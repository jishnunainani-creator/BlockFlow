...
export const DailyExecutionScoreCard: React.FC<Props> = ({ score, compact = false }) => {
  if (!score) {
    return (
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex items-center justify-center min-h-[200px]">
        <p className="text-slate-400 font-semibold text-sm">No data yet today</p>
      </div>
    );
  }
    ...
    <div className={`bg-slate-900 border border-slate-800 rounded-2xl p-6 relative overflow-hidden ${bgGlow}`}>
      ...
      <div className="relative z-10 flex flex-col items-center">
        ...
        <div className={`grid gap-3 w-full ${compact ? 'grid-cols-2' : 'grid-cols-3'}`}>
          ...
          <StatChip icon={<AlertCircle size={14} className="text-red-400"/>} label="High Pri" value={`${Math.round(priorityScores.high)}%`} />
          <StatChip icon={<AlertCircle size={14} className="text-amber-400"/>} label="Med Pri" value={`${Math.round(priorityScores.medium)}%`} />
          <StatChip icon={<AlertCircle size={14} className="text-emerald-400"/>} label="Low Pri" value={`${Math.round(priorityScores.low)}%`} />
          <StatChip icon={<Clock size={14} className="text-blue-400"/>} label="Time Acc" value={`${Math.round(timeAccuracyPct)}%`} />
